(ns actors.minimal-dag-system
  "Minimal Directed Acyclic Graph (DAG) System for ACTORS"
  (:require [clojure.core.async :as async :refer [go go-loop chan <! >! timeout]]
            [clojure.string :as str]
            [actors.simple-higher-order-functions :as hof]))

;; =============================================================================
;; MINIMAL DAG DATA STRUCTURES
;; =============================================================================
...
(defn demo-financial-dag []
  ...
  (let [dag (create-financial-dag)
    ...
    (let [result (execute-dag dag initial-data)]
      (println "   Execution order:" (:execution-order result))
      (println "   Success:" (:success result))
...
(defn run-all-minimal-dag-demos []
  (println "🎯 === ACTORS Minimal DAG System Comprehensive Demo ===")
  (println)
  ...
  (demo-financial-dag)
  (println)
  (println "🎉 === All Minimal DAG Demos Complete ==="))

(defn -main [& args]
  (run-all-minimal-dag-demos))
